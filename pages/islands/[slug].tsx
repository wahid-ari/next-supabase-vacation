...
export async function getServerSideProps(context: any) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/island?slug=${slug}&seo=true`).then((res) =>
    res.json(),
  );
  return {
    props: {
      ...
      seo: res,
      fallback: {
        [`${process.env.NEXT_PUBLIC_API_ROUTE}/api/island?slug=${slug}&seo=true`]: res,
      },
    },
  };
}

export default function Islands({ slug, seo }) {
  const { data, error } = useIslandData(null, slug);
  ...
  if (error) {
    return (
      <FrontLayout title='Island Detail - MyVacation' description='View Detail Island - MyVacation'>
