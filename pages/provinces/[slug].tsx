...
export async function getServerSideProps(context: any) {
  ...
  const res = await fetch(`${process.env.NEXT_PUBLIC_API_ROUTE}/api/province?slug=${slug}&seo=true`).then((res) =>
    res.json(),
  );
  return {
    props: {
      ...
      seo: res,
      fallback: {
        [`${process.env.NEXT_PUBLIC_API_ROUTE}/api/province?slug=${slug}&seo=true`]: res,
      },
    },
  };
}

export default function Provinces({ slug, seo }) {
  const { data, error } = useProvinceData(null, slug);
  ...
  if (error) {
    return (
      <FrontLayout title='Province Detail - MyVacation' description='View Detail Province - MyVacation'>
