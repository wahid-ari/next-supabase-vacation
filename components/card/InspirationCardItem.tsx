...
export default function InspirationCardItem({ className, alt = 'Name', image_url, onClick, ...props }: Props) {
  ...
  return (
    <button
      onClick={onClick}
      className={cn(
        'relative h-56 rounded-md focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-sky-500',
        className,
      )}
      {...props}
    >
      <Image
        ...
        src={image_url}
        fill
        className={cn('rounded-md object-cover', isLoading ? 'blur-sm' : 'blur-0')}
        unoptimized
